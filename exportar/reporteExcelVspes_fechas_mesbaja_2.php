...
tea.EstadoActual
	...
	left join dim_pacalificar_new_dh pvf on vf.SCCMVTFT=pvf.SCCMVTFT           
        ...
        order by a.iddim_Verificacion, vf.DNI_DH_0, pvf.InicioPCalificar1 asc";
...
//                ->setCategory("Reporte excel");
        ...
        $titulosColumnas = array('UCF/OSPE',
            ...
            'FIN DEL'.PHP_EOL.'PERIODO DE'.PHP_EOL.'BAJA'.PHP_EOL.'DH'.PHP_EOL.'(FECHA)',
...
//        $i = 10;
        while ($fila = $resultado->fetch_array()) {
            $objPHPExcel->setActiveSheetIndex(0)
                    ...
                    ->setCellValue('L' . $i, $fila['VINCULO_0_DES'])
                    ->setCellValueExplicit('M' . $i, $fila['DNI_DH_0'], PHPExcel_Cell_DataType::TYPE_STRING)
                    ->setCellValue('N' . $i, $fila['APELLIDO_NOMBRE_0'])
                    ->setCellValue('O' . $i, $fila['InicioPFinalDh'])                    
                    ->setCellValue('P' . $i, $fila['FinPFinalDh'])
                    
//                    ->setCellValueExplicit('P' . $i, $fila['codigocaso'], PHPExcel_Cell_DataType::TYPE_STRING)
                    ->setCellValue('Q' . $i, $fila['ordenV'])
                    ->setCellValue('R' . $i, $fila['fechaEmision'])
                    ->setCellValue('S' . $i, $fila['nit'])
                    ->setCellValue('T' . $i, $fila['fechaEIFinalJOSPE'])
                    ->setCellValue('U' . $i, $fila['fechaDevInfFinalJOSPE'])
                    ->setCellValue('V' . $i, $fila['fechaRDerivado'])
                    ->setCellValue('W' . $i, $fila['fechaDDerivado'])
                    ->setCellValue('X' . $i, $fila['oficina'])
                    ->setCellValue('Y' . $i, $fila['numResBOficio'])
                    ->setCellValue('Z' . $i, $fila['fechaEmiBOficio'])
                    
//                    ->setCellValue('Z' . $i, $fila['RRBRegistro'])
//                    ->setCellValue('AA' . $i, $fila['nResInhabilitacion'])
//                    ->setCellValue('AB' . $i, $fila['fechaEmiRInh'])
//                    ->setCellValue('AC' . $i, $fila['numRMulta'])
//                    ->setCellValue('AD' . $i, $fila['fechaNMulta'])
//                    ->setCellValue('AE' . $i, $fila['idTRRBRegistro'])
//                    ->setCellValue('AF' . $i, $fila['fecartafinanza1'])
//                    ->setCellValue('AG' . $i, $fila['ncartafinanza1'])
                    ->setCellValue('AA' . $i, $fila['apellidonombre'])
                    ->setCellValue('AB' . $i, $fila['observaciones']);
//                    ->setCellValue('AJ' . $i, $fila['EstadoActual']);
            $i++;
        }
...
//        $objPHPExcel->getActiveSheet()->getStyle('P3:AJ3')->applyFromArray($estiloTituloColumnas1);
