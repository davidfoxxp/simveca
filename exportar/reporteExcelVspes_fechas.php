...
if (isset($_POST['buscar'])) {
   ...
   // $cbx_estadoActual = $_POST['idOficinaUsuario'];
    $ano = $_POST['cbx_ano'];            
...
where month(a.fechaDevInfFinalJOSPE) in ($enero, $febrero, $marzo, $abril, $mayo, $junio, $julio, $agosto, $setiembre, $octubre, $noviembre, $diciembre) and 
year(a.fechaDevInfFinalJOSPE)='$ano'        
...
//        $i = 10;
        while ($fila = $resultado->fetch_array()) {
            $objPHPExcel->setActiveSheetIndex(0)
                    ->setCellValue('A' . $i, $fila['iddim_Verificacion'])
                    
                    ->setCellValue('B' . $i, $fila['OFICINA'])                   
                    ->setCellValue('C' . $i, $fila['VERIFICACION'])
                    ->setCellValue('D' . $i, $fila['VerificacionPerfil'])
                    ->setCellValue('E' . $i, $fila['EstadoActual'])
//                    ->setCellValueExplicitByColumnAndRow('E' . $i, $fila['dni_t'],utf8_encode(strval('dni_t')), PHPExcel_Cell_DataType::TYPE_STRING)
                    ->setCellValue('F' . $i, $fila['fechateinfoFinalVe'])
                    ->setCellValue('G' . $i, $fila['fechaEIFinalJOSPE'])
                    ->setCellValue('H' . $i, $fila['fechaDevInfFinalJOSPE'])
                    ->setCellValueExplicit('I' . $i, $fila['nit'], PHPExcel_Cell_DataType::TYPE_STRING)                   
                    ->setCellValue('J' . $i, $fila['fechaRDerivado'])                    
                    ->setCellValue('K' . $i, $fila['fechaDDerivado'])
                    
                    ->setCellValue('L' . $i, $fila['casoDerivado'])
                    ->setCellValue('M' . $i, $fila['iddim_VerificacionDerivado'])
                    ->setCellValue('N' . $i, $fila['codigocaso'])
                    ->setCellValue('O' . $i, $fila['observaciones'])
                    ->setCellValueExplicit('P' . $i, $fila['nombresUsuario'], PHPExcel_Cell_DataType::TYPE_STRING)
                    ->setCellValue('Q' . $i, $fila['fCreacion'])                    
                    ...
                    ->setCellValue('BD' . $i, $fila['APELLIDO_NOMBRE_0'])
                    ->setCellValue('BE' . $i, $fila['IniDh1'])
                    ->setCellValue('BF' . $i, $fila['FinDh1'])
                    ->setCellValue('BG' . $i, $fila['IniDh2'])
                    ->setCellValue('BH' . $i, $fila['FinDh2'])
                    ->setCellValue('BI' . $i, $fila['IniDh3'])
                    
                    ->setCellValue('BJ' . $i, $fila['FinDh3'])
                    ->setCellValue('BK' . $i, $fila['IniDh4'])
                    ->setCellValue('BL' . $i, $fila['FinDh4'])
                    ->setCellValue('BM' . $i, $fila['IniDh5'])
                    ->setCellValue('BN' . $i, $fila['FinDh5'])
                    ->setCellValue('BO' . $i, $fila['fCreacion'])
                    ->setCellValue('BP' . $i, $fila['nombresUsuario'])                    
                    ->setCellValue('BQ' . $i, $fila['APELLIDO_NOMBRE_0'])
                    ->setCellValue('BR' . $i, $fila['IniDh1'])
                    ->setCellValue('BS' . $i, $fila['FinDh1'])
                    
                    ->setCellValue('BT' . $i, $fila['IniDh2'])
                    ->setCellValue('BU' . $i, $fila['FinDh2']);
        ...
        $objPHPExcel->getActiveSheet()->getStyle('A3:BU3')->applyFromArray($estiloTituloColumnas);
        $objPHPExcel->getActiveSheet()->setSharedStyle($estiloInformacion, "A4:BU" . ($i - 1));
