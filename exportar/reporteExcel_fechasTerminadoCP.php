...
//                    ->setCellValue('T' . $i, $fila['fechanacimiento'])
                    ...
                    ->setCellValue('R' . $i, $fila['VINCULO_0_DES'])
                    ->setCellValueExplicit('S' . $i, $fila['DNI_DH_0'], PHPExcel_Cell_DataType::TYPE_STRING)
                    ->setCellValue('T' . $i, $fila['APELLIDO_NOMBRE_0'])
                    ->setCellValue('U' . $i, $fila['InicioPFinalDh'])                    
                    ->setCellValue('V' . $i, $fila['FinPFinalDh'])
                    
                    ->setCellValueExplicit('W' . $i, $fila['nombrePDF'], PHPExcel_Cell_DataType::TYPE_STRING)
                    ->setCellValue('X' . $i, $fila['prestaciones'])
                    ->setCellValue('Y' . $i, $fila['ncartafinanza1'])
                    
                    ->setCellValue('Z' . $i, $fila['observaciones'])
...
//        $objPHPExcel->getActiveSheet()->getStyle('AE3:AG3')->applyFromArray($estiloTituloColumnas55);
        $objPHPExcel->getActiveSheet()->setSharedStyle($estiloInformacion, "A2:AC" . ($i - 1));
...
//        $objPHPExcel->getActiveSheet(0)->freezePaneByColumnAndRow(0, 24);
        ...
        header('Content-Disposition: attachment;filename="Reporte_CP_BFyC_SIMVECA_Act_CDH.xlsx"');
