...
DATE_FORMAT(p.fCreacion, '%d/%m/%Y') fCreacion, 
...
//        $i = 10;
        while ($fila = $resultado->fetch_array()) {
            $objPHPExcel->setActiveSheetIndex(0)
                    ...
                    ->setCellValueExplicit('P' . $i, $fila['TipoAtencion'], PHPExcel_Cell_DataType::TYPE_STRING)                    
                    ->setCellValue('Q' . $i, $fila['paterno_t'])
                    ->setCellValue('R' . $i, $fila['materno_t'])
                    ->setCellValue('S' . $i, $fila['asegurado'])
                    
                    ->setCellValueExplicit('T' . $i, $fila['nombrePDPubli'], PHPExcel_Cell_DataType::TYPE_STRING)
...
//        $objPHPExcel->getActiveSheet()->getStyle('A2:F2')->applyFromArray($estiloTituloReporte1);
        ...
        $objPHPExcel->getActiveSheet()->getStyle('C1:M1')->applyFromArray($estiloTituloColumnas1);
        $objPHPExcel->getActiveSheet()->getStyle('N1:S1')->applyFromArray($estiloTituloColumnas2);        
        $objPHPExcel->getActiveSheet()->getStyle('T1:T1')->applyFromArray($estiloTituloColumnas3);        
        $objPHPExcel->getActiveSheet()->getStyle('U1:W1')->applyFromArray($estiloTituloColumnas7);
        $objPHPExcel->getActiveSheet()->setSharedStyle($estiloInformacion, "A2:W" . ($i - 1));
