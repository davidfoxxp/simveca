        ...
        $i = 4;
         ...
         while ($fila = $resultado->fetch_array()) {
            $objPHPExcel->setActiveSheetIndex(0)
                    ...
                    ->setCellValue('X' . $i, $fila['VINCULO_0_DES'])
                    ->setCellValueExplicit('Y' . $i, $fila['DNI_DH_0'], PHPExcel_Cell_DataType::TYPE_STRING)
                    ->setCellValue('Z' . $i, $fila['APELLIDO_NOMBRE_0'])
                    ->setCellValue('AA' . $i, $fila['InicioPFinalDh'])                    
                    ->setCellValue('AB' . $i, $fila['FinPFinalDh'])
                    
                    ->setCellValueExplicit('AC' . $i, $fila['nombrePDF'], PHPExcel_Cell_DataType::TYPE_STRING)
                    ->setCellValue('AD' . $i, $fila['ncartafinanza1'])
                    
                    ->setCellValue('AE' . $i, $fila['observaciones'])
...
//        $objPHPExcel->getActiveSheet()->getStyle('Z3:Z3')->applyFromArray($estiloTituloColumnas5);
        $objPHPExcel->getActiveSheet()->getStyle('M3:W3')->applyFromArray($estiloTituloColumnas3);
        $objPHPExcel->getActiveSheet()->getStyle('X3:AB3')->applyFromArray($estiloTituloColumnas6);
        $objPHPExcel->getActiveSheet()->getStyle('AC3:AE3')->applyFromArray($estiloTituloColumnas5);

        $objPHPExcel->getActiveSheet()->getStyle('AF3:AH3')->applyFromArray($estiloTituloColumnas7);
//        $objPHPExcel->getActiveSheet()->getStyle('AE3:AG3')->applyFromArray($estiloTituloColumnas55);
        $objPHPExcel->getActiveSheet()->setSharedStyle($estiloInformacion, "A4:AH" . ($i - 1));
        ...
        header('Content-Disposition: attachment;filename="Reporte_CP_FirmesyConsentidas_Const_CDH.xlsx"');
