...
//                order by cp.iddim_CPosterior, pc.InicioPCalificar1 asc";
    ...
    $consulta=  "SELECT cp.iddim_CPosterior,
                ...
                and ai.idDIM_Oficina='$iddimoficina' and cp.ruta_pdf is not null           
                and not cp.idtusuario_s=1
                order by cp.iddim_CPosterior, pc.InicioPCalificar1, vf.DNI_DH_0, pvf.InicioPCalificar1 asc";
    ...
    if ($resultado->num_rows > 0) {
       ...
       $tituloReporte = "RELACION DE CONTROL POSTERIOR FIRMES Y CONSENTIDAS POR LA OSPE";
        ...
        $objPHPExcel->setActiveSheetIndex(0)
                ...
                ->setCellValue('AE6', $titulosColumnas[30])
                ->setCellValue('AF7', $titulosColumnas[31])
                ->setCellValue('AG8', $titulosColumnas[32]);
        ...
        while ($fila = $resultado->fetch_array()) {
            $objPHPExcel->setActiveSheetIndex(0)
                    ...
                    ->setCellValue('W' . $i, $fila['VINCULO_0_DES'])
                    ->setCellValueExplicit('X' . $i, $fila['DNI_DH_0'], PHPExcel_Cell_DataType::TYPE_STRING)
                    ->setCellValue('Y' . $i, $fila['APELLIDO_NOMBRE_0'])
                    ->setCellValue('Z' . $i, $fila['InicioPFinalDh'])                    
                    ->setCellValue('AA' . $i, $fila['FinPFinalDh'])
                    

                    ->setCellValueExplicit('AB' . $i, $fila['nombrePDF'], PHPExcel_Cell_DataType::TYPE_STRING)
                    ->setCellValue('AC' . $i, $fila['ncartafinanza1'])                    
                            
                    ->setCellValue('AD' . $i, $fila['observaciones'])
...
//        $objPHPExcel->getActiveSheet()->getStyle('Y3:AD3')->applyFromArray($estiloTituloColumnas7);
        $objPHPExcel->getActiveSheet()->getStyle('Z3:AG3')->applyFromArray($estiloTituloColumnas55);
        ...
        header('Content-Disposition: attachment;filename="Reporte_CP_FirmesyConsentidas.xlsx"');
