...
----------------------------*/
	...
	if (empty($_POST['idcontrolposterior'])) {
           $errors[] = "ID vacío";
        } else if (empty($_POST['observaciones'])){
			$errors[] = "OBSERVACIONES";
		}    else if (
			!empty($_POST['idcontrolposterior']) &&
...
//		$continente=mysqli_real_escape_string($con,(strip_tags($_POST["continente"],ENT_QUOTES)));
		$id=intval($_POST['idcontrolposterior']);
		$sql="UPDATE dim_cposterior SET  observaciones='".$observaciones."' WHERE iddim_CPosterior='".$id."'";
