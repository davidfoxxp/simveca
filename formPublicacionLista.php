...
//        $iddim_Multa;
        ...
        $query = "SELECT ove.idDIM_Oficina,
                    p.iddim_Publicacion,
                    p.fpublicacion_p,
                    p.fpublicacion_e,
                    p.fpublicacion_c,
                    ...
                    left join dim_verificacion ve on p.iddim_Verificacion=ve.iddim_Verificacion
                    left join dim_overificacion ove on ve.iddim_Verificacion=ove.iddim_Overificacion
                    where p.iddim_Verificacion='$iddim_verificacion'";
                ...
                if ($conexionmysql->query($query)) {
                    while ($fila = $result->fetch_assoc()) {
                        ...
                        <tr style="background-color: #FFFFFF ; border-color:#87CEFA; text-align: center">
                            <td style="font-size: 13px"><?php echo $fila['fpublicacion_p'] ?></td>
                            <td style="font-size: 11px"><?php echo $fila['resolucionpublicada'] ?></td>
                            <?php  
                            if($fila['tipo_registro'] =='1'){
                                ?>
                                <td style="font-size: 13px">ASEGURADO</td>
                            <?php }
                            else { ?>
                                <td style="font-size: 13px">EMPLEADOR</td>
                            <?php }
                            ?>

                            <?php if (is_null($ruta_pdf_publi)) { ?>    
                                <td style="font-size: 11px"><?php echo $fila['nombrePDPubli'] ?></td>
                                <?php
                            } else {
                         ...
                         <td>                                               
                             <a href="#" id="abriPoppupp<?php echo $g ?>" style="font-size: 15px" title="Visualizar"><span class="glyphicon glyphicon-eye-open"></span>
                                ...
                                <div id="dialogg<?php echo $g ?>" title="DETALLE DE LA PUBLICACION" class="contenido">
                                    <iframe  src="formPublicacion_Visualizar.php?iddim_Publicacion=<?php echo $fila['iddim_Publicacion'] ?>" style="width: 100%; height: 100%"></iframe>
                                </div>
                    ...
                    <tr> 
                        <td colspan="2"><a href="#" id="abriPoppup10">Ingresar Publicacion</a>
                            <div id="dialog10" title="PUBLICAR" class="contenido">
                                <iframe src="formPublicacion.php?iddim_Verificacion=<?php echo $iddim_verificacion ?>" style="width: 100%; height: 100%"></iframe>
                            </div>
                        </td>
                    </tr>
                    <tr>
